...
<body>
    ...
    <div id="FondoFormularios">
        <?php 
          ...
          {
             $insercion = "INSERT INTO productos (`IdSucursal`,`RutaImagen`,`Codigo`,`Cantidad`,`Descripcion`,`Marca`,`PrecioCompra`,`PrecioVenta`,`PrecioVentaEspecial`,`G500`,`GEspecial`,`InventarioBajo`,`Medida`,`UnidadMedida`,`Comentario`) VALUES ('$Sucursal','$row[RutaImagen]','$row[Codigo]','$row[Cantidad]','$row[Descripcion]','$row[Marca]','$row[PrecioCompra]','$row[PrecioVenta]','$row[PrecioVentaEspecial]','$row[G500]','$row[GEspecial]','$row[InventarioBajo]','$row[Medida]','$row[UnidadMedida]','$row[Comentario]')";
             $resultadoInsercion = mysql_query($insercion);
             if ($contador == 0)
             {
              echo "Este es el insert: ".$insercion;
              $contador = 1;
             }
             if ($resultadoInsercion)
              $contador = $contador + 1;
          }
          echo "Se duplicaron: ".$contador." resultados";
        ?>
    </div>
    <div id="footer">
